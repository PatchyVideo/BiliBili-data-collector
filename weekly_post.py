...
def post_json(url, json_obj_or_string) :
	...
	cookies = {'session': '0409c0f2%2C1617462017%2Cb169e*a1'}
...
query = f"site:bilibili date:>=\"{week_time};00:00:00;GMT+8\" date:<\"{cur_time};00:00:00;GMT+8\""
print(f"Bilibili videos from {week_time} 00:00:00 GMT+8 to {cur_time} 00:00:00 GMT+8")
...
hours_count_map = defaultdict(int)
days_count_map = defaultdict(int)
...
for vid in videos :
    ...
    uploaddate = datetime.fromtimestamp(vid['item']['upload_time']['$date'] / 1000)
    uploaddate_gmt_8 = uploaddate.astimezone(tz=pytz.timezone('Asia/Shanghai'))
    hours_count_map[uploaddate_gmt_8.hour] += 1
    days_count_map[uploaddate_gmt_8.day] += 1
    video_class = set()
    for tag in tags :
        if tag in main_tags :
...
print("\n发布日期分析")
for (k, v) in days_count_map.items() :
    print(f'{k} => {v}')

print("\n发布时间分析")
for i in range(24) :
    print(f'{i} => {hours_count_map[i]}')
